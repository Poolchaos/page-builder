...
export class SignDocumentStore {
  ...
  handleUploadSelectedFile(action, evt) {
    ...
    logger.debug(' handleUploadSelectedFile ', evt);
    
    STATE.preview = null;
    var file = evt.target.files[0];
    logger.debug(' UPLOAD_SELECTED_FILE >>> file = ', file);
  ...
  handleSignDocument(action, multiple) {
    ...
    logger.debug(' setting multiple >>> SIGN_DOCUMENT_ACTIONS.SIGN_DOCUMENT >>> ', multiple);
  ...
  @handle(SIGN_DOCUMENT_ACTIONS.CLEAR_FILE)
  @handle(SIGN_DOCUMENT_ACTIONS.SELECT_FILE)
  handleSNavigateToSelectFile(action) {

    logger.debug(' setting multiple >>> ' + action + ' >>> 1');
    STATE.multiple = '1';
    STATE.document = null;
    STATE.signedDocument = null;
    STATE.preview = null;
    STATE.documentSignatureRequested = null;
  }
