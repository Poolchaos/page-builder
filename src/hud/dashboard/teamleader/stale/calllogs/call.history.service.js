...
import {searchParams} from './call.history.store';
/*
*/
const logger = LogManager.getLogger('AgentCallHistoryService');
/*
*/
...
export class AgentCallHistoryService {
  ...
  retrieveCallHistory(ownerId, page, size) {
    this.applicationService.displayAccountInteractionLogCallLog(ownerId, page, size)
        .then(
                response => {
                  if (response) {
                    this.dispatcher.dispatch(CALL_HISTORY_ACTIONS.CALL_HISTORY_RETRIEVED, response);
                  }
                },
                error => {
                  this.dispatcher.dispatch(CALL_HISTORY_ACTIONS.CALL_HISTORY_FAILED, error);
                });
  }
