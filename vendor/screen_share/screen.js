...
(function () {
  ...
  window.Screen = function (channel, errorCallback) {
  ...
  function Signaler(root, roomid) {
    ...
    function leaveRoom() {
      ...
      if (root.stream) {
        for(var i in root.stream.getTracks()) {
          root.stream.getTracks()[i].stop();
          root.stream.getTracks()[i].enabled = false;
        } 
      }
  ...
  function loadIceFrame(callback, skip) {
    ...
    iframe.onload = function () {
      ...
      if (!window.iceServers) return;
      callback(window.iceServers);
      //      });
  ...
  //
  //      window.iceServers = JSON.parse(xhr.responseText).d.iceServers;
  //
  //      if (!JSON.parse(xhr.responseText).d.iceServers) return;
  //
  //      callback(JSON.parse(xhr.responseText).d.iceServers);
  //    };
  //    xhr.onerror = function () {
  //      console.error("Woops, there was an error making xhr request.");
  //    };
  //    xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
  //    xhr.send("ident=muazkh&secret=6f52bc67-4c2d-445c-ac5d-6024fe7975fb&domain=webrtcexperiment-webrtc.netdna-ssl.com&application=default&room=default&secure=1");
  //
  //  }
